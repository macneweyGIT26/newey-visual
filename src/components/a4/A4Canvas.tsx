...
// A4 V5 — Rebuilt from Mashup baseline
// Motion core is identical to Mashup (known-good)
// Live data wired via runtime fetch (non-blocking)
...
// Fallback static colors if live data has no project/lane info
...
const FALLBACK_ORB_COLORS = ['34,211,238','168,85,247','251,146,60','253,186,116','250,204,21','196,181,253','147,197,253','236,72,153']
...
export default function A4Canvas() {
  ...
  const actRef    = useRef(0.5) // 0..1 activity; updated from tracker_entries on fetch
  ...
  useEffect(() => {
    ...
    // Non-blocking live data fetch — updates refs only, never re-renders
    const fetchLive = () => {
      fetch(`${LIVE_JSON_URL}?t=${Date.now()}`)
        ...
        .then((d: LiveData) => {
          ...
          if (d.tracker_entries != null) {
            actRef.current = Math.max(0.3, Math.min(1.0, d.tracker_entries / 100))
          }
        })
        .catch(() => {})
    }
    ...
    const initAll = () => {
      ...
      const w = W(), h = H()
      // 50 orbs — same as Mashup baseline
      for (let i = 0; i < 50; i++) {
        orbsRef.current.push({
          x: Math.random()*w, y: h*0.67+Math.random()*h*0.33,
          vx: (Math.random()-0.5)*0.12, vy: (Math.random()-0.5)*0.1,
          r: 25+Math.random()*80,
          color: FALLBACK_ORB_COLORS[Math.floor(Math.random()*FALLBACK_ORB_COLORS.length)],
    ...
    const draw = () => {
      ...
      if (t%5===0) {
        ...
        const col = laneColors[Math.floor(Math.random()*laneColors.length)]
        flowRef.current.push({
          ...
          color:col, width:2+Math.random()*3, glow:5+Math.random()*10,
        })
      }
      ...
      ctx.fillText(`${regime} · ${Math.round(act*100)}% active`,15,S.streetY+54)
      ...
      // Traffic — same spawn logic as Mashup, weighted by lane cost
      if (t%6===0 && trafficRef.current.length<90) {
        const lane = LANES[Math.floor(Math.random()*LANES.length)]
        const ly = S.streetY + S.streetH * lane.frac
        trafficRef.current.push({
          x:0, y:ly+(Math.random()-0.5)*15,
          vx:(lane.cost/totalCost)*2+0.3, vy:(Math.random()-0.5)*0.15,
      ...
      trafficRef.current.forEach(dot => {
        ...
        if (Math.random()<0.004) {
          ...
          else if(target>S.soulY) dot.color=FALLBACK_ORB_COLORS[Math.floor(Math.random()*FALLBACK_ORB_COLORS.length)]
      ...
      ctx.fillText(`${timestamp} · ${VERSION} · ${entries} entries`,w-12,15)
