...
// A5 — Agent Motion layer
// Runtime fetch live.json from GitHub; falls back to static build copy
// A5 adds named agent particles (Horner, Zack) with visible wandering motion
...
interface AgentP {
  x:number;y:number;vx:number;vy:number;color:string;size:number;phase:number
  name:string;pulseSpeed:number;targetX:number;targetY:number
...
export default function A5Canvas() {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      ctx.setTransform(2, 0, 0, 2, 0, 0) // reset to exactly 2x — prevents accumulation on repeated resize
      const h = H()
    ...
    const initAll = () => {
      ...
      // Agent particles — speed set high enough for visible motion
      const agentDefs = [
        { name: 'Horner', color: '166,107,255', pulseSpeed: 0.04, speed: 1.2 },
        { name: 'Zack', color: '34,197,94', pulseSpeed: 0.06, speed: 0.9 },
      ]
      agentDefs.forEach(ag => {
        agentRef.current.push({
          x: w * (0.2 + Math.random() * 0.6),
          y: S.streetY + S.streetH * (0.2 + Math.random() * 0.6),
          vx: (Math.random() - 0.5) * ag.speed,
          vy: (Math.random() - 0.5) * ag.speed,
          color: ag.color,
          ...
          targetY: S.streetY + Math.random() * S.streetH,
          trail: [],
          speed: ag.speed,
        })
      })
    ...
    const draw = () => {
      ...
      // ═══ AGENT PARTICLES — visible wandering motion ═══
      agentRef.current.forEach(agent => {
        // Pick new target when close or randomly (wander pattern)
        const distToTarget = Math.hypot(agent.targetX - agent.x, agent.targetY - agent.y)
        if (distToTarget < 20 || Math.random() < 0.005) {
          agent.targetX = w * (0.08 + Math.random() * 0.84)
          agent.targetY = S.streetY + 20 + Math.random() * (S.streetH - 40)
        }

        // Steer toward target at full speed — makes motion visible
        const dx = agent.targetX - agent.x
        const dy = agent.targetY - agent.y
        const dist = Math.hypot(dx, dy)
        if (dist > 0.1) {
          agent.vx = (dx / dist) * agent.speed * act
          agent.vy = (dy / dist) * agent.speed * act
        }
        ...
        // Hard boundary clamp
        agent.x = Math.max(w * 0.05, Math.min(w * 0.95, agent.x))
        agent.y = Math.max(S.streetY + 10, Math.min(S.soulY - 10, agent.y))

        // Trail — shows path of movement
        agent.trail.push({ x: agent.x, y: agent.y })
        if (agent.trail.length > 40) agent.trail.shift()

        agent.phase += agent.pulseSpeed
        const pulse = Math.sin(agent.phase) * 0.5 + 0.5
        const pulseSize = agent.size * (0.8 + pulse * 0.5)

        // Draw trail
        agent.trail.forEach((tr, ti) => {
          const a = (ti / agent.trail.length) * 0.25
          ctx.beginPath(); ctx.arc(tr.x, tr.y, agent.size * 0.35, 0, Math.PI * 2)
          ctx.fillStyle = `rgba(${agent.color},${a})`; ctx.fill()
        })

        // Glow halo
        const glow = ctx.createRadialGradient(agent.x, agent.y, 0, agent.x, agent.y, pulseSize + 14)
        glow.addColorStop(0, `rgba(${agent.color},${pulse * 0.5})`)
        glow.addColorStop(1, `rgba(${agent.color},0)`)
        ctx.beginPath(); ctx.arc(agent.x, agent.y, pulseSize + 14, 0, Math.PI * 2)
        ctx.fillStyle = glow; ctx.fill()

        // Core
        ctx.beginPath(); ctx.arc(agent.x, agent.y, pulseSize, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(${agent.color},${0.85 + pulse * 0.15})`; ctx.fill()

        // Name label
        ctx.font = '8px -apple-system, sans-serif'; ctx.textAlign = 'center'
        ctx.fillStyle = `rgba(${agent.color},0.7)`
        ctx.fillText(agent.name, agent.x, agent.y - pulseSize - 5)
      })
